...
class FeatureExtractor(object):
    ...
    def extract_features(tokens, buffer, stack, arcs):
        ...
        global printed
        if not printed:
            print("Let's see how your new features perform!")
            printed = True
        ...
        if stack:
            ...
            #Extra features added from Table 3.2 of Dependency parsing book
            if 'lemma' in token and FeatureExtractor._check_informative(token['lemma']):
                result.append('STK_0_LEMMA_' + token['lemma'])
            
            if 'postag' in token and FeatureExtractor._check_informative(token['ctag']):
                result.append('STK_0_POSTAG_' + token['postag'])
        ...
        if len(stack)>1:
            ...
            if 'postag' in token and FeatureExtractor._check_informative(token['postag']):
                result.append('STK_1_POSTAG_' + token['postag'])    
        ...
        if len(buffer)>2:
           ...
           if 'postag' in token and FeatureExtractor._check_informative(token['postag']):
                result.append('BUF_2_POSTAG_' + token['postag'])  
        ...
        if len(buffer)>3:
           ...
           if 'postag' in token and FeatureExtractor._check_informative(token['postag']):
                result.append('BUF_3_POSTAG_' + token['postag'])
